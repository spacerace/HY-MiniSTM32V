...
int gen_tasklist(char * pcWriteBuffer) {
	TaskStatus_t *pxTaskStatusArray;
	...
	*pcWriteBuffer = 0x00;
	...
	if( pxTaskStatusArray != NULL )
	{
		...
		for( x = 0; x < uxArraySize; x++ )
		{
            ...
            snprintf(namebuf, 13, "%12s", pxTaskStatusArray[ x ].pcTaskName);
			sprintf( pcWriteBuffer, "%02x %s %c %1x %5d %10ld %3ld\n",
                            ( unsigned int ) pxTaskStatusArray[ x ].xTaskNumber,
                            namebuf,
                            cStatus,
                            ( unsigned int ) pxTaskStatusArray[ x ].uxCurrentPriority, 
                            ( unsigned int ) pxTaskStatusArray[ x ].usStackHighWaterMark*4,
                            pxTaskStatusArray[ x ].ulRunTimeCounter*10, ulStatsAsPercentage
            );
            pcWriteBuffer += strlen( pcWriteBuffer );
        }
        
        sprintf( pcWriteBuffer, "               time total %10ld 100\n", ulTotalRunTime*10);
        pcWriteBuffer += strlen( pcWriteBuffer );
